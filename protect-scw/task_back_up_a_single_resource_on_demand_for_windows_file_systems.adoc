...
+
如果没有符合要求的现有策略、您可以复制并修改现有策略、也可以通过单击启动策略向导来创建新策略 image:../media/add_policy_from_resourcegroup.gif[""] 。

+
选定策略将在为选定策略配置计划部分的策略列中列出。

.. 在为选定策略配置计划部分中、单击 image:../media/add_policy_from_resourcegroup.gif[""] 要配置计划的策略的配置计划列。
