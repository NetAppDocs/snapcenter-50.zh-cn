...
** 本地管理员（仅适用于工作组）
+
对于属于工作组的系统、请指定要安装SnapCenter插件的系统上的内置本地管理员。 如果本地用户帐户具有提升的权限或主机系统上禁用了用户访问控制功能、则可以指定属于本地管理员组的本地用户帐户。用户名字段的有效格式如下： `UserName`
...
.. 运行 `Get-ADServiceAccount` 命令以验证服务帐户。
