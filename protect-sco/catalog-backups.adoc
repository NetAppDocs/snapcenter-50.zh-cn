...
如果在从 SnapCenter 图形用户界面创建 Oracle 备份策略时启用了编目选项，则在备份操作中会使用 Oracle RMAN 对备份进行编目。您也可以通过运行命令来执行延迟备份编目 `Catalog-SmBackupWithOracleRMAN` 。

编制备份目录后、您可以运行 `Get-SmBackupDetails` 命令来获取已编制目录的备份信息、例如、已编制目录的数据文件的标记、控制文件目录路径以及已编制目录的归档日志位置。
...
有关可与命令结合使用的参数及其说明的信息，可通过运行 get-help command_name 来获取。或者，您也可以参考 https://library.netapp.com/ecm/ecm_download_file/ECMLP2886896["《 SnapCenter 软件命令参考指南》"^]。
